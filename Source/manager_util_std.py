...
def get_generate_minutes_splited(path, input_file, minutes):
    ...
    ticks['temperature1'] = ticks['temperature1'].apply(lambda x: x / 40)
    ticks['temperature2'] = ticks['temperature2'].apply(lambda x: x / 40)
    ticks['luminosity'] = ticks['luminosity'].apply(lambda x: x * 100 / 2000)
    ticks['ground_humidity1'] = ticks['ground_humidity1'].apply(lambda x: x / 1023)
    ticks['ground_humidity2'] = ticks['ground_humidity2'].apply(lambda x: x / 1023)
    ticks['ground_humidity3'] = ticks['ground_humidity3'].apply(lambda x: x / 1023)
    ticks['environment_humidity1'] = ticks['environment_humidity1'].apply(lambda x: x / 100)
    ticks['environment_humidity2'] = ticks['environment_humidity2'].apply(lambda x: x / 100)
    ...
    ticks.to_csv(path + 'raw_data_minutes_std.csv', index=True, na_rep='N/A', columns=columns)
    return 'raw_data_minutes_std.csv'
...
def group_by_hours_temperature(path, input_file, minutes, time_behind_hours):
    ...
    result.to_csv(path + 'grouped-temperature-range-results_std.csv', index=False, header=False)
...
def group_by_hours_luminosity(path, input_file, minutes, time_behind_hours):
    ...
    result['end_time'] = pd.to_datetime(result['start_time']) + datetime.timedelta(minutes=60 * hours)
    result = result.dropna()
    result.to_csv(path + 'grouped-luminosity-range-results_std.csv', index=False, header=False)
...
def group_by_hours_ground_humidity(path, input_file, minutes, time_behind_hours):
    ...
    result['end_time'] = pd.to_datetime(result['start_time']) + datetime.timedelta(minutes=60 * hours)
    result = result.dropna()
    result.to_csv(path + 'grouped-ground-humidity-range-results_std.csv', index=False, header=False)
...
def group_by_hours_environment_humidity(path, input_file, minutes, time_behind_hours):
    ...
    result['end_time'] = pd.to_datetime(result['start_time']) + datetime.timedelta(minutes=60 * hours)
    result = result.dropna()
    result.to_csv(path + 'grouped-environment-humidity-range-results_std.csv', index=False, header=False)
